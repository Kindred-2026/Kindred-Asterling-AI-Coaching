...
A read-only GitHub metadata query returned repository-level Actions secret **names only**; no values were accessed. After checking canonical `main` and the finalization branch workflows, unused `CLERK`, `NEON_API_KEY`, and `NEON_PROJECT_ID` entries were removed on 2026-09-24. PR #164 merged on 2026-09-24, removed the OpenCode comment bot, and its sole-consumer secret `OPENCODE_API_KEY` was then deleted. A names-only query on 2026-09-25 returned only `SNYK_TOKEN`; a refresh after PR #190 still returns only `SNYK_TOKEN`, and repository Actions variables remain empty. PRs #183, #184, #185, #187, #189, and #190 are merged; the canonical baseline is now `e696630`. Environment-level secret and variable names were refreshed on 2026-09-25 and are listed below. No secret values were accessed. This is a point-in-time inventory, not proof that the retained Snyk token is correctly scoped.
