...
| `OPENAI_API_KEY` | S; OpenAI-compatible provider auth; Cloudflare account REST uses a Cloudflare runtime token, required when `AI_PROVIDER=openai` | local secret | no OpenAI-compatible credential found in current production | Fly staging secret staged 2026-09-26; Cloudflare token `kindred-fly-staging-ai`, Workers AI Read, restricted to the Kindred account; no GitHub copy | retain for staging; revoke/reissue on exposure or staging retirement; account-scoped, not gateway-restricted |
...
| `CLOUDFLARE_AI_GATEWAY_ID` | N; selects the named Cloudflare gateway via `cf-aig-gateway-id`; optional, used only for recognized Cloudflare endpoints | local config | not configured | Fly config; `kindred-staging` gateway has authentication, privacy, rate and budget controls | retain; remove with Cloudflare integration; no credential rotation required |
...
### GitHub repository inventory — names checked 2026-09-26

A read-only GitHub metadata query returned repository-level Actions secret **names only**; no values were accessed. After checking canonical `main` and the finalization branch workflows, unused `CLERK`, `NEON_API_KEY`, and `NEON_PROJECT_ID` entries were removed on 2026-09-24. PR #164 merged on 2026-09-24, removed the OpenCode comment bot, and its sole-consumer secret `OPENCODE_API_KEY` was then deleted. A names-only query on 2026-09-26 returned only `SNYK_TOKEN`; repository Actions variables remain empty. PRs #183, #184, #185, #187, #189, and #190 are merged; the code/config inventory baseline remains `e696630`. Names-only queries on 2026-09-26 found no secrets or variables in any of the four repository environments. No secret values were accessed. This is a point-in-time inventory, not proof that the retained Snyk token is correctly scoped.
