...
A read-only GitHub metadata query returned repository-level Actions secret **names only**; no values were accessed. After checking canonical `main` and the finalization branch workflows, unused `CLERK`, `NEON_API_KEY`, and `NEON_PROJECT_ID` entries were removed on 2026-09-24. PR #164 merged on 2026-09-24, removed the OpenCode comment bot, and its sole-consumer secret `OPENCODE_API_KEY` was then deleted. A fresh names-only query on 2026-09-25, against canonical `main` at `4b724ff`, again returned only `SNYK_TOKEN`. The latest CI, Security Audit, and Snyk runs for that SHA passed, and there are no open PRs. Repository variables and all three environment secret/variable lists were last checked on 2026-09-24 and were empty. This is a point-in-time inventory, not proof that the retained Snyk token is correctly scoped.
