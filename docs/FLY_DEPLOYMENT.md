...
| AI configuration | Fly remains `AI_PROVIDER=disabled`, without an AI credential or endpoint. Read-only inspection found existing production configured for Bedrock, with no OpenAI-compatible key or endpoint to reuse. Cloudflare account/model/budget inspection is awaiting an accessible dashboard session; no inference or quality pass is claimed |
