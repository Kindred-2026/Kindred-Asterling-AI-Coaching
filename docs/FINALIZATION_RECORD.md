...
| GitLab CI disposition | Owner confirmed GitLab is unused; legacy pipeline file removed after repository audit. GitHub Actions is the sole active repository CI path. Historical GitLab references retained only as records. Obsolete active-sounding Phase 3 specs/prompt marked historical; `scripts/ci-run.mjs` comment made provider-neutral. | GitLab API returned 401; project/pipeline state and Coolify connection not inspected. Duplicate-pipeline cleanup gate closed. |
