...
| --- | --- | --- |
| Experimental Next.js `frontend/` | No `frontend/` application, Next.js dependency, workspace package, or standalone CI path remains. `scripts/dev-supervisor.test.mjs` intentionally keeps a regression assertion that no supervisor job targets `next`. The production React/Vite app remains under `artifacts/kindred-coach`. | Confirmed by current root tree, package manifests, workflow scan, and the passing production build in PR #187; the negative regression assertion is retained as a guard |
...
| GitHub branch cleanup | Deleted the merged PR #145 branch after verifying its tip was an ancestor of canonical `main`; archived all 16 unique historical branch tips under verified GitHub tags, then deleted their stale branch names. PR #147 removed the confirmed-unused AWS EKS assets and merged at `49e0be1`. PR #149 merged the finalization and Snyk work; redundant PR #148 was closed without merge after verifying its workflow changes were included. | On 2026-09-25, PRs #184 and #185 merged. Their remote branches and the merged local PR #184 branch were removed after ancestry checks. Closed PR #186's `APP_PUBLIC_URL` correction is present in `main` and the tracked `fly.toml` now uses `https://kindred-asterling-ai-coaching.fly.dev`; its duplicate branch was deleted. `git ls-remote --heads origin` then listed only `main`. The merged, clean `auth0-deploy-integration` worktree and local branch were removed after verifying PR #142 and its commit were in `main`. No unique commits were lost; historical tip dispositions remain in [the branch cleanup record](BRANCH_CLEANUP.md) |
