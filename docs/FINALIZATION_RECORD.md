...
**Baseline:** GitHub `main` at `e91135e5d774abc700235adcef3d5ac14cbdb79d` (2026-09-24, after merge of PR #155).
...
| GitLab CI disposition | `.gitlab-ci.yml` remains a complete validation, test, build, and security pipeline; it contains no deploy job. The canonical checkout has only the GitHub remote, and GitLab CLI authentication is unavailable | Active GitLab project/pipeline state and any connection to the legacy Coolify release source could not be checked (GitLab API returned 401). Keep the file until GitLab and Coolify usage are confirmed; the duplicate-pipeline cleanup gate is still open |
...
| Repository secrets | Value-free inventory; repository-level Actions secrets are `OPENCODE_API_KEY` and `SNYK_TOKEN`; repository variables are empty. The three named GitHub environments have no environment-level secrets or variables | The signed-in GitHub organization Actions settings pages show no organization-level secrets or variables. Historical environment deployment records remain; verify out-of-band purposes before deleting environments or rotating runtime credentials |
...
| GitHub branch cleanup | Deleted the merged PR #145 branch after verifying its tip was an ancestor of canonical `main`; archived all 16 unique historical branch tips under verified GitHub tags, then deleted their stale branch names. PR #147 removed the confirmed-unused AWS EKS assets and merged at `49e0be1`. PR #149 merged the finalization and Snyk work; redundant PR #148 was closed without merge after verifying its workflow changes were included. | The live GitHub branch inventory was checked after cleanup and contains only `main`. The remote branches for PRs #148 and #149 were deleted. No unique commits were lost; all 16 historical tips have dispositions in [the branch cleanup record](BRANCH_CLEANUP.md) |
| Local checkout cleanup | Local `main` is aligned to canonical `e91135e`; former unique commit is preserved at local archive ref `archive/kindred-local-main-40c8841`. Removed the clean PR #148, #149, and #155 worktrees after their changes were incorporated; preserved the separate Auth0 migration checkout with local changes and the invalid Copilot worktree registration | No unique change from the old commit was ported because its gitlink, privacy statement, and workspace placeholder were invalid or superseded. User-owned Auth0 edits and the invalid Copilot worktree remain untouched; see [workspace checkout audit](WORKSPACE_CHECKOUTS.md) |
