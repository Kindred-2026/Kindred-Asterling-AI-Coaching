...
| GitHub branch cleanup | Deleted the merged PR #145 branch after verifying its tip was an ancestor of canonical `main`; archived all 16 unique historical branch tips under verified GitHub tags, then deleted their stale branch names. PR #147 removed the confirmed-unused AWS EKS assets and merged at `49e0be1`. PR #149 merged the finalization and Snyk work; redundant PR #148 was closed without merge after verifying its workflow changes were included. | After PR #164, later merged work branches and the three audited remediation branches were deleted after their changes were confirmed in `main`. After PR #181 and pruning on 2026-09-24, `git ls-remote --heads origin` listed only `main`. No unique commits were lost; all 16 archived historical tips have dispositions in [the branch cleanup record](BRANCH_CLEANUP.md) |
| Local checkout cleanup | The initial finalization checkout was aligned to canonical `e91135e`; its former unique commit remains at local archive ref `archive/kindred-local-main-40c8841`. The primary checkout is canonical `main` at `e9f41e8`; untracked `.vscode/` is preserved. The PostgreSQL rehearsal worktree is clean at the same SHA on local branch `codex/postgres-rehearsal`, with dependencies retained for the credentialed test. The separate Auth0 migration checkout has local edits and remains untouched; the invalid Copilot worktree registration remains untouched | No unique change from the old commit was ported because its gitlink, privacy statement, and workspace placeholder were invalid or superseded. See [workspace checkout audit](WORKSPACE_CHECKOUTS.md) |
...
Canonical source is GitHub `main`, currently `e9f41e8` after PR #181. The Fly
...
The staging app has no image, deployment, or machines. Managed Postgres cluster
