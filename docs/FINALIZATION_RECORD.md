...
| Repository secrets | Value-free inventory; current repository Actions secrets are `OPENCODE_API_KEY` and `SNYK_TOKEN`; repository variables are empty. The three named GitHub environments have no environment-level secrets or variables, and the organization has none. | PR #164 removes the OpenCode bot's sole consumer; delete `OPENCODE_API_KEY` after merge. Retain `SNYK_TOKEN` for the active security workflow. Historical environment deployment records remain; verify out-of-band purposes before deleting environments or rotating runtime credentials |
...
| GitHub branch cleanup | Deleted the merged PR #145 branch after verifying its tip was an ancestor of canonical `main`; archived all 16 unique historical branch tips under verified GitHub tags, then deleted their stale branch names. PR #147 removed the confirmed-unused AWS EKS assets and merged at `49e0be1`. PR #149 merged the finalization and Snyk work; redundant PR #148 was closed without merge after verifying its workflow changes were included. | After PRs #161–#163 merged, the live GitHub branch API showed `main` and open PR #164 only. Fifteen stale local `origin/*` tracking refs were pruned; no live remote branch was deleted. No unique commits were lost; all 16 archived historical tips have dispositions in [the branch cleanup record](BRANCH_CLEANUP.md) |
| Local checkout cleanup | The initial finalization checkout was aligned to canonical `e91135e`; its former unique commit remains at local archive ref `archive/kindred-local-main-40c8841`. The current primary checkout is on the PR #164 branch based on canonical `main` at `17262dd`; untracked `.vscode/` is preserved. The separate Auth0 migration checkout remains user-owned and modified, and the invalid Copilot worktree registration remains untouched | No unique change from the old commit was ported because its gitlink, privacy statement, and workspace placeholder were invalid or superseded. See [workspace checkout audit](WORKSPACE_CHECKOUTS.md) |
