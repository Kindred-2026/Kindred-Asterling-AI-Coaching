...
| GitHub branch cleanup | Deleted the merged PR #145 branch after verifying its tip was an ancestor of canonical `main`; archived all 16 unique historical branch tips under verified GitHub tags, then deleted their stale branch names. PR #147 removed the confirmed-unused AWS EKS assets and merged at `49e0be1`. | The verified 2026-09-24 inventory contains `main`, `codex/snyk-iac-no-files` (PR #148), and `codex/remove-dead-clerk-release-docs` (PR #149). PR #149 now targets `main` and includes the same Snyk workflow fix as #148 plus the finalization cleanup; if #149 is merged, #148 becomes redundant and should be closed without a separate merge. No unique commits were lost; all 16 historical tips have dispositions in [the branch cleanup record](BRANCH_CLEANUP.md) |
